...
class DeterministicViewTests(unittest.TestCase):
    ...
    def _implementation_option(
        ...
        *,
        ...
        return Record(
            record_type="implementation_option",
            front_matter={
                "id": record_id,
                "name": "Option|name\\path\ncontinued",
                "status": status,
                "implements": ["QOL-1000", "QOL-999"],
                "acquisition": "purchase",
                "deprecation_reason": "Superseded" if status == "Deprecated" else None,
                "replaced_by": ["IMP-1000"] if status == "Deprecated" else [],
            },
            body="",
        )
    ...
    def test_implementation_option_view(self):
        snapshot = RepositorySnapshot(
            categories=(),
            items=(),
            references=(),
            implementation_options=(
                self._implementation_option("IMP-1000", status="Deprecated"),
                self._implementation_option("IMP-999"),
            ),
        )

        rendered = views.render_implementation_options(snapshot)

        self.assertTrue(rendered.startswith("<!-- Generated by python -m qol_kb.views. Do not edit. -->\n"))
        self.assertLess(
            rendered.index("## Active Implementation Options"),
            rendered.index("## Deprecated Implementation Options"),
        )
        self.assertIn("[IMP-999](../implementation-options/IMP-999.md)", rendered)
        self.assertIn("[QOL-999](../items/QOL-999.md)", rendered)
        self.assertIn("[QOL-1000](../items/QOL-1000.md)", rendered)
        self.assertLess(rendered.index("QOL-999"), rendered.index("QOL-1000"))
        self.assertIn("| purchase |", rendered)
        self.assertIn("Option\\|name\\\\path continued", rendered)
        self.assertIn("Superseded", rendered)
        self.assertIn("[IMP-1000](../implementation-options/IMP-1000.md)", rendered)
        self.assertEqual(rendered, views.render_implementation_options(snapshot))
        self.assertTrue(rendered.endswith("\n"))
        self.assertFalse(rendered.endswith("\n\n"))

    def test_implementation_option_view_uses_fixed_empty_states(self):
        rendered = views.render_implementation_options(RepositorySnapshot((), (), ()))

        self.assertIn("No Active Implementation Options.", rendered)
        self.assertIn("No Deprecated Implementation Options.", rendered)
