...
class CanonicalCutoverTests(unittest.TestCase):
    ...
    def test_catalog_and_reference_indexes_are_direct_generated_outputs(self):
        self.assertEqual(
        ...
        for relative_path, required_section in (
            (Path("catalog.md"), b"## Active QoL Items"),
            (Path("references.md"), b"## Active References"),
        ):
            ...
            self.assertIn(required_section, committed)
    ...
    def test_topic_views_reject_legacy_registry_identity_links(self):
        snapshot = records.load_repository(REPOSITORY_ROOT)
        selections = views.load_topic_selections(REPOSITORY_ROOT)
        source = (REPOSITORY_ROOT / "topics" / "attention-digital.md").read_text(
            encoding="utf-8"
